...
The progression badge maps maximum completed kingdom level `1/2/3` to bronze/silver/gold. The same badge also draws a visible level mark for the latest completed kingdom level, capped at Level 4, so a completed Level 4 remains visible even though the color stays gold. Stars on the progression badge come only from completed survival loops saved in `kingdom_survival_rounds`; do not draw stars from ordinary completed levels.
