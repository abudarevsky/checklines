...
- Light Trap is the neon trap-library behavior: it uses `assets/sprites/lite_trap.png` in the trap cloud, marks the replacement cell with a spawn-whirl-like laser action area, targets a member of a nearly completed line, shows the same trap cloud/history flow as a capture, replaces the targeted piece with a random type/color identity not currently present elsewhere on the board, avoids the threatened line's matching color/type so that line breaks, and spawns one additional random piece on a random empty cell.
